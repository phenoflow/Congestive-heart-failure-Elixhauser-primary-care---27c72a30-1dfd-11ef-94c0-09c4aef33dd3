...
(async () => {
  ...
  await fs.writeFile("congestive-heart-failure-elixhauser-primary-care-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("congestive-heart-failure-elixhauser-primary-care-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
